...
class App:
    ...
    def _run_wizard(self) -> None:
        source = filedialog.askdirectory(title="Select state/source imagery folder")
        if not source:
            return
        ...
        zooms = detect_zoom_dirs(source_dir)
        if not zooms:
            messagebox.showerror("No zoom folders found", f"No numeric zoom folders found under:\n{source_dir}")
            return

        out_parent = filedialog.askdirectory(title="Select output parent folder")
        if not out_parent:
            return
        ...
        custom_name = derive_output_name(source_dir)

        provider = simpledialog.askstring(
            "Provider name",
            "Enter provider/source name stored in the DB:\n(Leave as default unless you know ATAK expects something else)",
            initialvalue=source_dir.name,
            parent=self.root,
        )
        if not provider:
            return
        ...
        self.config = BuildConfig(
            ...
            sqlite_path=output_dir / custom_name,
            provider=provider,
        )

        self.summary_var.set(
            f"Detected zooms: {', '.join(map(str, zooms))} | Output: {self.config.sqlite_path} | Log: {log_file}"
        )
        self.status_var.set("Running...")
        self.append_log("=" * 80)
        self.append_log(f"Starting build at {datetime.now().isoformat(timespec='seconds')}")
        self.append_log(f"Auto ATAK folder: {output_dir.name}")
        self.append_log(f"Auto output filename: {custom_name}")
