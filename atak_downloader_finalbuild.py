...
- GUI workflow
- Checkbox zoom selection (all deselected by default)
- Human-readable resolution hints
- Automatic state-boundary download to a temp folder
- Progress bar during download
- Safe re-run: skips tiles that already exist
- Unified logging to terminal + GUI + ./logs/
...
USER_AGENT = "ATAK-Ortho-Downloader/1.0"
...
def load_states(geojson_path: Path) -> Dict[str, List[List[Tuple[float, float]]]]:
    """
    Returns:
        {state_name: [ring1, ring2, ...]}
    Each ring is a list of (lon, lat) tuples.
    Supports Polygon and MultiPolygon outer rings only.
    """
...
# -----------------------------
# Geometry helpers (no shapely/geopandas needed)
# -----------------------------
...
class ZoomDialog(tk.Tk):
    def __init__(self) -> None:
        ...
        note = (
            "Higher zoom = more detail and much larger downloads.\n"
        ...
        for z in range(10, 17):
            ...
            self.vars[z] = var
            cb = tk.Checkbutton(checks, text=zoom_resolution_labels(z), variable=var, anchor="w", justify="left")
            cb.pack(anchor="w")
...
def prompt_state() -> str:
    root = tk.Tk()
    root.withdraw()
    value = tk.simpledialog.askstring(APP_TITLE, "Enter a state abbreviation (e.g. AZ) or ALL:")
    root.destroy()
    if not value:
        return ""
...
def ask_create_dir() -> bool:
    root = tk.Tk()
    root.withdraw()
    answer = messagebox.askyesno(APP_TITLE, "Create a new output directory inside the selected parent folder?")
    root.destroy()
    return bool(answer)
...
def run_download(selected_zooms: List[int], state_input: str, output_parent: Path, create_dir: bool, progress: ProgressWindow) -> None:
    ...
    try:
        ...
        if state_input == "ALL":
            state_names = sorted([s for s in states if s != "District of Columbia"])
            output_root = output_parent / "All_States" if create_dir else output_parent
        else:
            state_name = STATE_ABBR_TO_NAME.get(state_input, state_input.title())
            if state_name not in states:
                raise RuntimeError(f"State not found in boundary file: {state_input}")
            state_names = [state_name]
            output_root = output_parent / state_name if create_dir else output_parent

        output_root.mkdir(parents=True, exist_ok=True)
        log(f"Using output root: {output_root}")

        # Plan tiles first so the progress bar is real
...
def main() -> None:
    ...
    state_input = prompt_state()
    if not state_input:
        log("Cancelled at state prompt.")
        return
    ...
    worker = threading.Thread(
        target=run_download,
        args=(selected_zooms, state_input, Path(output_folder), create_dir, progress),
        daemon=True,
    )
    worker.start()
    progress.mainloop()
